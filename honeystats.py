...
def process_staking_events(registry, error_counter, chain_name):
    ...
    while from_block <= current_block:
        ...
        try:
            ...
            # Save last processed block number
            with open(LAST_BLOCK_FILE_STAKING, 'w') as f:
                json.dump({chain_name: to_block}, f)
...
def main(registry):
    ...
    query_threads = []
    for chain_name in CHAINS:
        query_thread = threading.Thread(target=query_contract_metrics, args=(registry, chain_name))
...
def query_contract_metrics(registry, chain_name):
    chain_config = CHAINS[chain_name]
